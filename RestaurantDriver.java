...
public class RestaurantDriver {
    ...
    private static void showMenuKoki(Pegawai koki) {
        while (true) {
            ...
            switch (pilihan) {
                ...
                case 2:
                    tampilkanDaftarPesananKoki();
                    if (system.getPesananByStatus(RestaurantSystem.STATUS_MENUNGGU).isEmpty()) {
                        break;
                    }
                    ...
                    try {
                        ...
                        if (pesanan != null && pesanan.getStatus().equals(RestaurantSystem.STATUS_MENUNGGU)) {
                            // ubah ke STATUS_DIPROSES atau langsung selesaikan sesuai flow; gunakan STATUS_SELESAI
                            koki.updateStatusPesanan(pesanan, RestaurantSystem.STATUS_SELESAI);
                            system.simpanSemuaDataPesanan();
                        } else if (pesanan == null) {
                            System.out.println("Pesanan tidak ditemukan.");
                        } else {
                            System.out.println(
                                    "Status pesanan ini tidak dapat diubah (Status: " + pesanan.getStatus() + ").");
                        }
    ...
    private static void tampilkanDaftarPesananKoki() {
        System.out.println("\n--- Pesanan Perlu Dimasak (Status: " + RestaurantSystem.STATUS_MENUNGGU + ") ---");
        List<Pesanan> pesananDipesan = system.getPesananByStatus(RestaurantSystem.STATUS_MENUNGGU);
        ...
        while (true) {
            ...
            switch (pilihan) {
                case 1:
                    ...
                    if (system.getPesananByStatus(RestaurantSystem.STATUS_SELESAI).isEmpty()) {
                        break;
                    }
                    prosesPembayaran();
                    break;
    ...
    private static void tampilkanDaftarSiapBayar() {
        System.out.println("\n--- Pesanan Siap Bayar (" + RestaurantSystem.STATUS_SELESAI + ") ---");
        List<Pesanan> pesananSiapBayar = system.getPesananByStatus(RestaurantSystem.STATUS_SELESAI);
    ...
    private static void buatPesananBaru() {
        ...
        for (Meja m : system.getDaftarMeja()) {
            if (m.getStatus().equals(RestaurantSystem.MEJA_KOSONG)) {
        ...
        if (mejaPilihan == null || !mejaPilihan.getStatus().equals(RestaurantSystem.MEJA_KOSONG)) {
            System.out.println("Meja tidak valid atau tidak tersedia.");
            return;
        }
        java.util.List<DetailPesanan> daftarDetail = new java.util.ArrayList<>();
        while (true) {
            system.lihatMenu();
            ...
            try {
                ...
                daftarDetail.add(new DetailPesanan(item, jumlah, catatan));
        ...
        Pesanan pesananBaru = system.buatPesanan(mejaPilihan, daftarDetail);
        if (pesananBaru != null) {
            System.out.println("Pesanan baru (ID: " + pesananBaru.getIdPesanan() + ") untuk Meja "
                    + mejaPilihan.getNomor() + " berhasil dibuat.");
        } else {
    ...
    private static void prosesPembayaran() {
        ...
        try {
            ...
            if (pesanan.getStatus().equals(RestaurantSystem.STATUS_DIBAYAR)) {
                System.out.println("Pesanan ini sudah lunas.");
                return;
            }
            if (!pesanan.getStatus().equals(RestaurantSystem.STATUS_SELESAI)) {
                System.out.println("Pesanan ini belum siap dibayar (Status: " + pesanan.getStatus()
                        + "). Harap Koki selesaikan dulu.");
                return;
            }
            ...
            system.updateStatusPesanan(idPesanan, RestaurantSystem.STATUS_DIBAYAR);
            system.simpanSemuaDataPesanan();
